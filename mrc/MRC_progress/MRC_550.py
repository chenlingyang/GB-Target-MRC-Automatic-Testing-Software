...
class MRC550Processor:
    ...
    MASK_OPEN_SIZE = 5
    # 550 基准剖面：单行最大方差 + σ 平滑后差分（非 CLAHE/锐化/dense 分峰）
    PROFILE_LINE_SMOOTH_SIGMA = 1.5
    PROFILE_DENSE_GROUP_MIN = 19   # 19+ 组检测 σ=0（条纹密度高，σ=1.5 会导致峰谷粘连）
    PROFILE_DENSE_MID_GROUP_MIN = 21   # 21+ 组统一做亮区裁边
    PROFILE_DENSE_SIGMA = 0.0          # 21–22 组检测用 σ=0（不抹峰谷）
    PROFILE_DENSE_PEAK_SIGMA = 0.0     # 23+ 组检测用 σ=0
    # C_mean 测量用 σ（保持与旧方法一致，确保跨组可比）
    PROFILE_MEASURE_SIGMA_DENSE = 1.0    # 21+ 组 C_mean 测量统一用 σ=1.0，消除 σ 差异造成的跨组跳变
    PROFILE_PEAK_PROMINENCE_RATIO = 0.06
    PROFILE_PEAK_MIN_PITCH_RATIO = 0.48
    ...
    @staticmethod
    def _average_profile_rows(roi_gray: np.ndarray, row_idx: int, num_rows: int = 3) -> np.ndarray:
        """取 row_idx 附近 num_rows 行的列均值：降噪同时保留水平条纹细节。

        相比单行高斯平滑，垂直平均不会模糊横向相邻峰谷，对密集条纹更友好。
        """
        h = roi_gray.shape[0]
        half = num_rows // 2
        r_start = max(0, row_idx - half)
        r_end = min(h, row_idx + half + 1)
        return np.mean(roi_gray[r_start:r_end, :], axis=0).astype(np.float32)

    @classmethod
    def _profile_smooth_sigma(cls, group_id: Optional[int] = None) -> float:
        """检测用 σ：密集组用 0 保证峰谷不粘连。"""
        if group_id is None:
            return float(cls.PROFILE_LINE_SMOOTH_SIGMA)
        gid = int(group_id)
        if gid >= int(cls.PROFILE_DENSE_MID_GROUP_MIN):
            return float(cls.PROFILE_DENSE_PEAK_SIGMA)
        if gid >= int(cls.PROFILE_DENSE_GROUP_MIN):
            return float(cls.PROFILE_DENSE_SIGMA)
        return float(cls.PROFILE_LINE_SMOOTH_SIGMA)

    @classmethod
    def _profile_measure_sigma(cls, group_id: Optional[int] = None) -> float:
        """C_mean 测量用 σ：21+ 组统一用 σ=1.0，消除 σ 差异造成的跨组跳变。"""
        if group_id is None:
            return float(cls.PROFILE_LINE_SMOOTH_SIGMA)
        gid = int(group_id)
        if gid >= 21:
            return float(cls.PROFILE_MEASURE_SIGMA_DENSE)
        return float(cls.PROFILE_LINE_SMOOTH_SIGMA)

    @classmethod
    def _trim_profile_line_bright_core(cls, line: np.ndarray) -> Tuple[np.ndarray, int, int]:
        """裁掉剖面两端低灰度（黑边），仅保留亮条纹 span。"""
        if line.size < 8:
            return line, 0, int(line.size)
        lo = float(np.percentile(line, 5))
        hi = float(np.percentile(line, 95))
        span = max(hi - lo, 1.0)
        thr = lo + span * 0.12
        active = np.where(line >= thr)[0]
        if active.size < 6:
            return line, 0, int(line.size)
        left = max(0, int(active[0]) - 1)
        right = min(int(line.size), int(active[-1]) + 2)
        if right - left < 8:
            return line, 0, int(line.size)
    ...
    def _filter_extrema_by_pitch(
        ...
        *,
        ...
        for i in idxs:
            ...
            neigh = np.concatenate([line[lo:i], line[i + 1 : hi]]) if hi - lo > 1 else line
            base = float(np.min(neigh)) if want_peak else float(np.max(neigh))
            prom = float(line[i]) - base if want_peak else base - float(line[i])
            if prom >= prominence:
                scored.append((prom, i))
        scored.sort(key=lambda x: (-x[0], x[1]))
        kept: List[int] = []
        for _prom, i in scored:
    ...
    def _apply_dense_peak_filters(
        ...
        lo = float(np.percentile(line, 5))
        hi = float(np.percentile(line, 95))
        prom = max((hi - lo) * float(cls.PROFILE_PEAK_PROMINENCE_RATIO), 2.5)
        exp_n = max(int(expected_stripes), 8)
        pitch = max(2.0, float(line.size) / float(exp_n))
        min_dist = max(2, int(round(pitch * float(cls.PROFILE_PEAK_MIN_PITCH_RATIO))))
        fp = cls._filter_extrema_by_pitch(line, peaks, min_dist=min_dist, prominence=prom, want_peak=True)
        fv = cls._filter_extrema_by_pitch(line, valleys, min_dist=min_dist, prominence=prom, want_peak=False)
        return fp, fv
    ...
    def _find_peaks_scipy(
        ...
        """scipy.signal.find_peaks：对密集条纹用 distance+prominence 约束，比差分法更鲁棒。"""
        from scipy.signal import find_peaks

        n = int(line.size)
        exp_n = max(int(expected_stripes), 8)
        pitch = float(n) / float(exp_n)
        distance = max(2, int(round(pitch * 0.35)))
        lo = float(np.percentile(line, 5))
        hi = float(np.percentile(line, 95))
        prominence = max(1.0, (hi - lo) * 0.025)
        peaks, _ = find_peaks(line, distance=distance, prominence=prominence)
        valleys, _ = find_peaks(-line, distance=distance, prominence=prominence)
    ...
    def _detect_peaks_valleys(
        ...
        exp_n = int(expected_stripes or 0)

        peaks, valleys = cls._diff_peaks_valleys(line)

        # 密集组（21+）：先做节距/幅度过滤；若仍偏离预期较多则回退到 scipy find_peaks
        if gid >= 21 and exp_n > 0:
            raw_n = len(peaks) + len(valleys)
            fp, fv = cls._apply_dense_peak_filters(line, peaks, valleys, exp_n)
            filt_n = len(fp) + len(fv)
            if abs(filt_n - exp_n) <= abs(raw_n - exp_n):
                peaks, valleys = fp, fv
            # 过滤后仍偏差 > 3 条 → scipy 回退
            cur_n = len(peaks) + len(valleys)
            if abs(cur_n - exp_n) > 3:
                try:
                    sp, sv = cls._find_peaks_scipy(line, exp_n)
                    if abs(len(sp) + len(sv) - exp_n) < abs(cur_n - exp_n):
                        return sp, sv
                except Exception:
                    pass
    ...
    def _extract_profile_metrics(
        ...
        """最大方差行 → 多行平均(21+) / σ 平滑 → 峰谷检测（21+ 组加节距/幅度过滤）→ C_mean。"""
        ...
        gid = int(group_id) if group_id is not None else 0
        if gid >= int(cls.PROFILE_DENSE_GROUP_MIN):
            # 密集组：用多行平均替代单行，降噪不损横向峰谷分离
            line_raw = cls._average_profile_rows(roi_gray, row_idx, num_rows=3)
        else:
            line_raw = roi_gray[row_idx, :].astype(np.float32)

        if gid >= int(cls.PROFILE_DENSE_MID_GROUP_MIN):
            line_raw, _left, _right = cls._trim_profile_line_bright_core(line_raw)

        # --- 检测线：用检测 σ（21+ 为 0，保证峰谷不粘连）---
        line_detect = cls._smooth_profile_line(line_raw, cls._profile_smooth_sigma(group_id))
        peaks, valleys = cls._detect_peaks_valleys(line_detect, group_id, expected_stripes)

        # --- 测量线：用测量 σ（与旧方法一致，保证 C_mean 跨组可比）---
        line_measure = cls._smooth_profile_line(line_raw, cls._profile_measure_sigma(group_id))
        peak_vals = [float(line_measure[i]) for i in peaks]
        valley_vals = [float(line_measure[i]) for i in valleys]
        pair_n, c_mean, peak_vals, valley_vals = cls._pair_michelson_c_mean(peak_vals, valley_vals)

        return {
            "line_raw": line_measure,
            "row_idx": row_idx,
            "peaks": peaks,
            "valleys": valleys,
            "pair_n": pair_n,
    ...
    def process_image(
        ...
        for m in sorted([x for x in mapped_rects if int(x["rect_id"]) == 1], key=lambda x: int(x["group_id"])):
            ...
            if gid >= int(self.PROFILE_DENSE_GROUP_MIN):
