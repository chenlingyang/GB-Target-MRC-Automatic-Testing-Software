    ...
    public sealed class TeledyneDalsaSaperaLtCaptureModule : ICaptureModule
    {
        ...
        public bool Initialize(DeviceSettings settings)
        {
            ...
            try
            {
                ...
                TryRouteSapManagerErrorsToLog(sapManagerType);
                TryDetectCameraLinkServers(sapManagerType);
        ...
        public bool StartCapture()
        {
            ...
            try
            {
                ...
                _snapshotLoopTask = Task.Run(() => SnapshotLoop(_captureCts.Token));
        ...
        public void StopCapture()
        {
            ...
            try
            {
                _snapshotLoopTask?.Wait(TimeSpan.FromMilliseconds(4000));
            }
            catch { }
        ...
        /// <summary>Camera Link：刷新服务器枚举（官方 FindCamera 示例用法）。</summary>
        private static void TryDetectCameraLinkServers(Type sapManagerType)
        {
            try
            {
                Type? detectEnum = sapManagerType.GetNestedType("DetectServerType", BindingFlags.Public);
                MethodInfo? m = sapManagerType.GetMethod(
                    "DetectAllServers",
                    detectEnum != null ? new[] { detectEnum } : Type.EmptyTypes);
                if (m == null || detectEnum == null)
                {
                    return;
                }

                object all = Enum.Parse(detectEnum, "All");
                m.Invoke(null, new[] { all });
                Thread.Sleep(1500);
            }
            catch
            {
        ...
        /// <summary>减少 SapLt 模态错误框，写入日志查看器。</summary>
        private static void TryRouteSapManagerErrorsToLog(Type sapManagerType)
        {
            try
            {
                PropertyInfo? prop = sapManagerType.GetProperty(
                    "DisplayStatusMode",
                    BindingFlags.Public | BindingFlags.Static);
                Type? nested = sapManagerType.GetNestedType("StatusMode", BindingFlags.Public);
                if (prop != null && nested != null && nested.IsEnum)
                {
                    object logVal = Enum.Parse(nested, "Log");
                    if (prop.PropertyType.IsAssignableFrom(nested))
                    {
                        prop.SetValue(null, logVal);
                    }
                }
            }
            catch
        ...
        /// <summary>销毁采集链路（不卸载 SapClassBasic），供初始化重试时调用。</summary>
        private void DestroySaperaPipeline()
        {
            DestroyObject(_sapTransfer);
            DestroyObject(_sapBuffer);
            DestroyObject(_sapAcquisition);
            DestroyObject(_sapLocation);
            _sapTransfer = null;
            _sapBuffer = null;
