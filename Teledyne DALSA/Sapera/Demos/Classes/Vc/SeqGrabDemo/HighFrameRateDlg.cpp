...
CHighFrameRateDlg::CHighFrameRateDlg(CWnd* pParent /* = NULL */, DWORD nFramesPerCallback /* = 1 */, DWORD nFramesOnBoard /* 2 */, SapTransfer* phXfer)
	: CDialog(CHighFrameRateDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CHighFrameRateDlg)
	m_nFramesPerCallback = nFramesPerCallback;
	m_nFramesOnBoard     = nFramesOnBoard;
	//}}AFX_DATA_INIT
...
void CHighFrameRateDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_SPIN_NFRAMES_ONBOARD, m_nFramesOnBoardSpinCtrl);
	...
	DDX_Text(pDX, IDC_NFRAMES_ONBOARD, m_nFramesOnBoard);
	DDV_MinMaxDWord(pDX, m_nFramesOnBoard, 1, 32767);
	//}}AFX_DATA_MAP
}
...
BOOL CHighFrameRateDlg::OnInitDialog() 
{
	...
	m_nFramesOnBoardSpinCtrl.SetRange( 2, 32767);
